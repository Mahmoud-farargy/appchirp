...
Vue.component("app-todolist",{
    ...
    <div style="min-height:100%;" class="toDoInner">
        ...
        <template>
            <li v-for="(toDo,itemIndex) in toDoList">
                <div class="toDoDeck">
                    <input name="item(itemIndex)" type="checkbox"  @click="sendToDoneList(itemIndex)" class="mr-2 ml-3" style="cursor:pointer;"><i @click="removeToDo(itemIndex)" class="fas fa-trash-alt mr-2" style="color:rgb(100, 30, 30); cursor:pointer;"></i> <span class="theItem">{{toDo}}</span>
        ...
        <template v-if="showFinishedItems">
            <li v-for="(doneItem, doneItemIndex) in thingsDoneList">
                <div class="finishedDeck ">
                    <input checked name="item(doneItemIndex)" @click="itemsUnfinished(doneItemIndex)" type="checkbox" class="mr-2 ml-3 " style="cursor:pointer;"><i @click="removeDoneItems(doneItemIndex)" class="fas fa-trash-alt mr-2" style="color:rgb(100, 30, 30); cursor:pointer;"></i><span class="theItems"><span class="things-done-style">{{doneItem}}</span></span> 
    ...
    methods:{
        ...
        updateServerData(){
              this.$http.put("https://mahmoudvue.firebaseio.com/todoList.json", this.toDoList)
              .then(request=>{
                console.log(request);
              }, error=>{
                console.log(error);
              ...
              this.$http.put("https://mahmoudvue.firebaseio.com/doneList.json", this.thingsDoneList)
              .then(request=>{
                console.log(request);
              }, error=>{
                console.log(error);
    ...
    created(){
        ...
        //receive ThingsDoneList array from the server and return it to the app array
            ...
            this.$http.get("https://mahmoudvue.firebaseio.com/doneList.json")
            .then(response=>{
               return response.json();
            })
            .then(data =>{
                const receivedToDoData= [];
                receivedToDoData.push(data);
                this.thingsDoneList = receivedToDoData[0];
                 console.log(receivedToDoData);
            })
    }
});
